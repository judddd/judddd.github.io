...
class Post
  ...
  def self.create(path, metaweblog_struct)
    ...
    filename=metaweblog_struct['link']+".md" || now.strftime('%Y-%m-%d-%H-%M-%S.md')
    dirname=metaweblog_struct['categories'] || 'default'
    path=path+"/"+dirname[0]
    unless Dir.exist?(path)
     Dir.mkdir(path)
     puts "Folder '#{dirname}' created successfully."
    else
     puts "Folder '#{dirname}' already exists."
    end
